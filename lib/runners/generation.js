...
module.exports = function (strategies, num_generations) {
  ...
  var formatted_strategies = Object.keys(strategies).map(function (name) {
    return [name, strategies[name]];
  });
  // now, create a ton of copies
  var NUM_COPIES = 5;
  var initial_population = [];
  for (var i = 0; i < NUM_COPIES; i++) {
    initial_population = initial_population.concat(copy(formatted_strategies));
  }
